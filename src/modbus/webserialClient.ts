...
class AsyncMutex {
	...
	release(): void {
		if (this.waiters.length > 0) {
			const resolve = this.waiters.shift() as () => void;
			resolve();
		} else {
			this.locked = false;
		}
...
export class WebSerialModbusClient {
	...
	constructor(
		...
		console.info(`${this.debugPrefix} initialized`, {
	...
	private calculateMinInterval(): number {
		...
		const silentIntervalMs =
			(bitsPerChar * 5 * 1000) / this.serialSettings.baudRate;
	...
	setPrecisionMode(isExtended: boolean): void {
		...
		this.minMessageIntervalMs = this.calculateMinInterval();
		console.info(
			`${this.debugPrefix} minMessageIntervalMs updated`,
			this.minMessageIntervalMs,
		);
	}
	...
	async connect(): Promise<boolean> {
		...
		const portInfoReason =
			portInfo === undefined ? "no info from getInfo()" : undefined;
	...
	async disconnect() {
		...
		if (this.reader) {
			...
			try {
				...
				console.warn(`${this.debugPrefix} reader cancel failed`, err);
			}
			try {
				this.reader.releaseLock();
			} catch (err) {
				console.warn(`${this.debugPrefix} reader releaseLock failed`, err);
			}
			this.reader = null;
		}
		...
		if (this.writer) {
			...
			try {
				...
				console.warn(`${this.debugPrefix} writer close failed`, err);
			}
			this.writer = null;
		}
		...
		if (this.port) {
			...
			try {
				...
				console.warn(`${this.debugPrefix} port close failed`, err);
			}
			this.port = null;
		}
	...
	private async flushReceiveBuffer(maxFlushMs?: number): Promise<void> {
		...
		while (true) {
			...
			const readResult =
				await Promise.race<ReadableStreamReadResult<Uint8Array> | null>([
					reader.read(),
			...
			if (readResult === null) {
				...
				try {
					...
					console.debug(
						`${this.debugPrefix} flushReceiveBuffer() cancel failed`,
						cancelError,
					);
				}
				try {
					...
					console.debug(
						`${this.debugPrefix} flushReceiveBuffer() releaseLock failed`,
						releaseError,
					);
				}
				if (this.port.readable) {
					try {
						...
						console.warn(
							`${this.debugPrefix} flushReceiveBuffer() getReader failed`,
							getReaderError,
						);
						this.reader = null;
						await this.disconnect();
		...
		if (discardedBytes > 0) {
			console.warn(`${this.debugPrefix} flushed stale RX bytes`, {
				discardedBytes,
			});
		}
	}
	...
	private async transfer(
		...
		try {
			const writer = this.writer as WritableStreamDefaultWriter<Uint8Array>;
			...
			const receivedCrc =
				responseArray[responseArray.length - 2] |
				(responseArray[responseArray.length - 1] << 8);
			...
			try {
				...
				console.warn(
					`${this.debugPrefix} transfer() flush after error failed`,
					flushErr,
				);
				if (this.reader && this.port?.readable) {
					try {
	...
	async readCoils(start: number, count: number): Promise<boolean[]> {
		...
		console.debug(`${this.debugPrefix} readCoils() done`, {
			responseByteCount,
	...
	async readHoldingRegisters(start: number, count: number): Promise<number[]> {
		console.debug(`${this.debugPrefix} readHoldingRegisters()`, {
			start,
			count,
		});
		const payload = [start >> 8, start & 0xff, count >> 8, count & 0xff];
	...
	async readInputRegisters(
		...
		console.debug(`${this.debugPrefix} readInputRegisters()`, {
			start,
			count,
			timeoutMs,
		});
	...
	async readInputRegistersAsFloat32Abcd(
		...
		console.debug(`${this.debugPrefix} readInputRegistersAsFloat32Abcd()`, {
			start,
			count,
			timeoutMs,
		});
		...
		console.debug(
			`${this.debugPrefix} readInputRegistersAsFloat32Abcd() done`,
			{
				byteCount,
				valuesLength: values.length,
				preview: values.slice(0, 10),
	...
	async writeSingleRegister(address: number, value: number): Promise<void> {
		console.debug(`${this.debugPrefix} writeSingleRegister()`, {
			address,
			value,
		});
	...
	async writeMultipleCoils(start: number, values: boolean[]): Promise<void> {
		console.debug(`${this.debugPrefix} writeMultipleCoils()`, {
			start,
			valuesLength: values.length,
		});
		if (values.length === 0) {
	...
	async writeMultipleHoldingRegisters(
		...
		if (values.length > 123) {
			throw new Error(
				"Cannot write more than 123 registers in a single request",
			);
		}
