...
function clampToViewport(geometry: WindowGeometry): WindowGeometry {
	...
	const x = Math.max(
		VIEWPORT_MARGIN,
		Math.min(geometry.x, window.innerWidth - width - VIEWPORT_MARGIN),
	);
	const y = Math.max(
		VIEWPORT_MARGIN,
		Math.min(geometry.y, window.innerHeight - height - VIEWPORT_MARGIN),
	);
...
export function FloatingWindow({
	...
	return (
		<div className="pointer-events-none fixed inset-0" style={{ zIndex }}>
			...
			>
				{/* biome-ignore lint/a11y/useSemanticElements: react-rnd requires a div, not <dialog> */}
				<div
					role="dialog"
					aria-label={title}
