...
const RUNNER_GUIDELINES: string[] = [
  ...
  'If a channel the script needs has no label below, or its identity is otherwise uncertain, do not guess: strongly urge the user to add a label for it in the app and to calibrate it before writing the script — at minimum every AI and AO channel the script touches. Where possible, urge them to put the unit in the label itself (e.g. "Force (N)", "Stroke [mm]"), since the Phy value display carries no unit of its own. A script written against an unlabeled or uncalibrated channel can only be checked by running it on the rig.',
