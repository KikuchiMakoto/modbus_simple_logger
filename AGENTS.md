...
│   ├── AppStatusBar.tsx             # 画面下端固定のアプリ状態／エラーバー。PyScript バーの上に重なる。何も無ければ null（スペーサを持たない純オーバーレイ）・全ブレークポイントで表示
...
└── utils/
    ...
    ├── appStatus.ts                 # アプリ状態／エラーのモジュールレベルストア（同一文言の畳み込み・source 別クリア・error は notify() へ）
...
- **配色ルール（重要）**: 明示的な指示がない限り、新規 UI 要素の色指定は **他と同じ緑（emerald）か通常のグレー（slate）のみ**を使う。青(blue/sky)・琥珀(amber)・赤(red)などを新規に持ち込まない。
  ...
  - **例外（画面下端バー系のみ）**: `ScriptStatusBar` / `AppStatusBar` の**エラー表示**に限り red を使ってよい（ドット `bg-red-500`、文字 `text-red-600 dark:text-red-400`）。`ScriptStatusBar` が `error` 判定と stderr で既にこのトークンを使っており、その兄弟コンポーネントが同じ意味に別の色を当てると「下端バーの赤＝異常」という既に成立している対応が壊れるため。**これは red を他の新規要素へ持ち込む許可ではない** — バー以外でエラーを表したくなったら、まずこのルールに戻ること。`AppStatusBar` の `dataLoss` レベルが amber なのは下の「取り戻せないデータ」例外の側で読む
