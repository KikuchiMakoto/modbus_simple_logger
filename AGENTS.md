...
- **通信レートと記録レートは別物**（v4.1〜）。Sampling Rate の選択は**記録（チャート/IndexedDB/TSV）の周期**であり、線上のポーリング周期は `MODBUS_POLL_INTERVAL_CAP_MS`(100ms) が上限 = `pollIntervalMsFor()` = `min(選択値, 100ms)`。200ms 以降は「2回に1回記録」「3000回に1回記録」という間引きで実現する
  - **理由は FB 制御**。AO を叩くスクリプトは polling が更新した AI 値を読む。ここを保存周期に縛ると、「ディスクには1分に1点で十分だが制御は速く回したい」という真っ当な要求が「1分に1回しか入力が更新されない制御ループ」になる。ファイルサイズと制御品質は別の関心事なので、**この2つを再び1つのレートに戻さないこと**
  - 50ms だけは上限より速いので、ポーリングも記録も 50ms（キャップは `min` なので何もしない）
  ...
  - 締切は **接続時・ポーリング再開時・保存開始/停止時に 0 へリセット**して「次のポーリングを即記録」に戻す。特に保存開始 — 5分周期で締切を引き継ぐと、ファイルの1行目が5分後になる
  - **AI チャネルカードは記録の有無に関わらず毎ポーリング更新する**（`enqueueDisplayUpdate` の `record` 引数は履歴系だけを止める）。5分周期の記録に合わせて読み値まで止めたら、画面は5分間死んだ数字を出し続ける
  - 読取りタイムアウト・リトライ可否（`canRetry`）も**ポーリング周期**基準。キャップにより実質どの設定でもリトライは無効だが、1フレーム落ちの代償は「1回のポーリング」であって「1点の記録」ではないので、締切方式と合わせて実害はない
  - ヘッダー表示は `Polling: 実測ms` = **線上のレート**（未計測は `-`）。公称値を併記しないのは、通信レートがユーザーの選択ではなく固定の定数になったから — 隣に 100 と出しても定数を読み上げるだけ。記録側の進捗は同じ行の点数カウンタが示す
