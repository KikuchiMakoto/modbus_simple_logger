...
| **Param Editor** | Parameter 用の**16ch × ラベル + 値**の手動編集窓。値は入力しただけでは変わらず、**横の `Set`（または Enter）で反映**します（Escape で取り消し）。表示は**入力した通りの桁**に戻ります（内部は float32 なので、`0.3` が `0.3000000152` に化けないよう「読み戻して同じ値になる最短表記」で出しています）。ラベルはメインページの Parameter グリッドと同じもの。最下部の**スライダで16ch の値とラベルを一括クリア**。ScriptRunner 実行中はロック（スクリプトと手入力が同じ Parameter を奪い合わないため。`Accept Risk` で値だけ解放でき、ラベルは解放されません） |
