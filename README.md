...
| **Script Log** | スクリプトの `print()` 出力・エラー・トレースバックを読むための独立ウィンドウ（メニュー → Script Log）。ScriptRunner の隣に並べて必要なだけ高くできます。末尾追従は「末尾を見ている間だけ」で、遡って読んでいる最中に引き戻されません。Copy でログ全体をクリップボードへ |
