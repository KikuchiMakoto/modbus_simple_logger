...
| **Param Editor** | Parameter 用の**16ch × Default + Present** の手動編集窓。**Present** は今の値で、入力を確定した瞬間に反映されます。**Default** は次回起動時に入る値で、編集しても**今の値は変わりません**（端末ごとに記憶）。ScriptRunner 実行中はロック（スクリプトと手入力が同じ Parameter を奪い合わないため） |
