...
- **設定の永続化** — アプリは固定ポート 8376（ASCII の "S"+"L"）で配信されるためオリジンが起動間で安定し、Calibration 値・チャート軸選択・ラベル等が exe の再起動をまたいで保持されます。8376 が他ソフトで占有されていた場合はランダムポートにフォールバックし、そのセッションは System Log に警告を出した上で設定の保存なしで動作します。
