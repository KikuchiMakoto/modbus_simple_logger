...
| **リモート監視** | デスクトップ版限定。同じ LAN 上の他 PC のブラウザから、チャートとチャネル値を**閲覧のみ**できます（既定オフ） |
...
計測中の画面を、同じ LAN 上の別の PC のブラウザから見られます。立ち会いの人に画面を見せたい、装置の隣を離れて様子を見たい、といった用途を想定しています。

**使い方**

1. 計測している PC で、メニューの **Remote Monitoring** を開きます。
2. **Share this screen on the LAN** をオンにします。
3. 表示された `http://192.168.x.x:8766/...` の URL を、見る側の PC のブラウザで開きます。
...
- **接続元は `192.168.*.*` に限定**しています。それ以外のアドレスからは、ファイルひとつ配信されず 403 になります。
- **URL の `?k=` は毎回変わります。** アプリを再起動すると古いリンクは無効になります。
- **平文の HTTP です。** 「この LAN の中の人になら見られてよい」範囲で使ってください。インターネット越しに見せる用途は想定していません。
