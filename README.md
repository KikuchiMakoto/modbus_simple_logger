...
| **ScriptRunner** | **Python / BASIC / Lua** をパネル上のセレクタで切り替えて実行（Web Worker + SharedArrayBuffer）。読み取りは `GetAiRaw()` / `GetAiPhy()` / `GetAo()` / `GetParam()`、書き込みは `SetAo()` / `SetParam()` / `SetAiTare()`、`SetNotify()` で通知。**API 名は3言語で共通**（全 API 一覧はパネル内にも表示）。**どの言語でも Stop はいつでも効きます**（出口の無いループの中でも） |
