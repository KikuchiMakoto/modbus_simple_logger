...
- 出力先は `launcher/bin/`（`launcher/` は gitignore 対象のため成果物はコミットされません）。
- Windows: `launcher/bin/modbus_simple_logger.exe` を生成（アイコン付き）。PE サブシステムを GUI に設定するため、起動時にコンソールウィンドウは一切表示されません。
- Linux: `launcher/bin/modbus_simple_logger` を生成（Edge → Chrome → Chromium の順に探索）。
- 起動後 1〜2 秒でブラウザのアプリウィンドウが開きます。ウィンドウを閉じるとランチャーは自動終了し、プロセスは残りません。
